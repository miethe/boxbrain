...
const AssetActions: React.FC<AssetActionProps> = ({ asset }) => {
...
export const AssetRow: React.FC<{ asset: Asset; onClick: () => void; showGrip?: boolean }> = ({ asset, onClick, showGrip = false }) => (
  <div onClick={onClick} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-lg hover:shadow-sm transition-shadow group cursor-pointer">
    ...
    <div className={`p-2 rounded flex flex-col items-center justify-center w-12 h-12 flex-shrink-0 ${
      asset.kind === 'deck' ? 'bg-orange-50' : 
...
export const AssetCard: React.FC<{ asset: Asset; onClick: () => void }> = ({ asset, onClick }) => {
