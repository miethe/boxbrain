...
export const AssetLibrary: React.FC<AssetLibraryProps> = ({ assets, dictionary, onViewAsset, onAddAsset }) => {
                ...
                {viewMode === 'grid' ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        ...
                        {assetsInGroup.map((asset, idx) => (
                             <AssetCard key={`${asset.id}-${group}-${idx}`} asset={asset} onClick={() => onViewAsset(asset.id)} />
                        ))}
                    </div>
                ) : (
                    <div className="space-y-2">
                        ...
                        {assetsInGroup.map((asset, idx) => (
                            <AssetRow key={`${asset.id}-${group}-${idx}`} asset={asset} onClick={() => onViewAsset(asset.id)} />
                        ))}
                    </div>
                )}
             </div>
